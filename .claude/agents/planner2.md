---
name: planner2
description: Planner with Chrome + Serena (experimental - no tools allowlist)
model: claude-opus-4-6
---

# Planner Agent (Experimental)
...
**This is the experimental variant.** It has no `tools:` allowlist, so all tools are inherited (Serena, Chrome, built-ins). See ADR-0003 for context.
...
- Review starter created at `.kit/context/<TASK-ID>-REVIEW-STARTER.md`
- Evaluator output persisted at `.kit/context/reviews/<TASK-ID>-evaluator-review.md`
...
**Review Starter Files**: Implementation agents create these to provide context for human reviewer. Template at `.kit/context/templates/review-starter-template.md`.
...
- **Review Starters**: `.kit/context/<TASK-ID>-REVIEW-STARTER.md` (created by implementation agents)
- **Evaluator Reviews**: `.kit/context/reviews/<TASK-ID>-evaluator-review.md`
...
3. **Estimate PR size** -- if >500 additions, add `## PR Plan` section to task spec (ADR-0002)
...
## Branch Isolation Policy (DSP-0037 lesson)
...
|----------|--------|-----------|
| Task specs, handoffs, evaluator triage | `main` | Prep materials (ADR-0002) |
...
#    [X.Y.Z]: https://github.com/movito/dispatch-kit/compare/vPREV...vX.Y.Z
...
7. **PR Size Workflow**: `.kit/context/workflows/PR-SIZE-WORKFLOW.md` (ADR-0002)
...
adversarial architecture-planner .kit/tasks/2-todo/TASK-FILE.md

# Task plan evaluation (fast, cheap):
adversarial architecture-planner-fast .kit/tasks/2-todo/TASK-FILE.md
...
assigned, or status updated), emit a completion event if dispatch-kit is installed:

```bash
dispatch emit phase_complete \
  --agent planner \
  --task $TASK_ID \
  --summary "<brief summary of coordination work completed>"
```

This is a no-op if dispatch-kit is not installed. Do not fail or warn if the
