...
The agentive-starter-kit uses a launcher script (`.kit/launchers/launch`) that passes the entire agent markdown file as a system prompt via `--append-system-prompt`. Large agent definitions can increase request processing time and make API overload errors more likely.
...
1. **Externalize verbose content**: Move detailed instructions to `.kit/context/` files that agents read on demand
...
- `.kit/launchers/launch` - Agent launcher script
- `.kit/launchers/onboarding` - First-run setup launcher
