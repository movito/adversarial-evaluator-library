...
|------|---------------|---------------------|
| `.kit/context/workflows/RESEARCH-QUALITY-STANDARDS.md` | Primary source | `adversarial evaluate`, evaluator names |
...
grep -r "adversarial evaluate" .claude/agents/ .kit/context/workflows/

# Find all files referencing specific evaluators
grep -r "gpt52-reasoning\|mistral-content\|o3-chain" .claude/agents/ .kit/context/

# Count coupled files
grep -l "adversarial evaluate" .claude/agents/*.md .kit/context/workflows/*.md | wc -l
```
