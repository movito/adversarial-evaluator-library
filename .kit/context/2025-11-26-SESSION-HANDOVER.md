...
1. **Onboarding Separation** (commit `c8b69c7`)
   ...
   - `.kit/launchers/onboarding` launches onboarding agent, not planner
...
2. **Serena MCP Integration** (commit `3f0057b`)
   ...
   - Updated `.kit/launchers/launch` to check if Serena is configured
...
| `.kit/launchers/onboarding` | Now launches onboarding agent |
...
### User Flow
1. Clone repo → `./.kit/launchers/onboarding` (first-time setup)
2. Onboarding agent guides through 6 phases including Serena
3. After setup → `./.kit/launchers/launch` for daily use
